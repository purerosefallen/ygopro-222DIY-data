...
function c65010015.initial_effect(c)
	...
	e4:SetOperation(c65010015.flagop)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	...
	e6:SetOperation(c65010015.setflag)
	c:RegisterEffect(e6)
end
...
function c65010015.flagfil(c,e)
	return c:IsFacedown() and c:GetFlagEffect(65010015)==0 
end

function c65010015.setflag(e,tp,eg,ep,ev,re,r,rp)
	local phase=Duel.GetCurrentPhase()
	if (phase==PHASE_DAMAGE and not Duel.IsDamageCalculated()) or phase==PHASE_DAMAGE_CAL then return end
	local g=Duel.GetMatchingGroup(c65010015.flagfil,tp,LOCATION_SZONE,0,nil,e)
	if g:GetCount()>0 then
		local gc=g:GetFirst()
		while gc do
			gc:RegisterFlagEffect(65010015,RESET_EVENT+RESETS_STANDARD,0,1) 
			gc=g:GetNext()
		end
		Duel.Readjust()
	end
...
function c65010015.flagop(e,tp,eg,ep,ev,re,r,rp)
	local ge1=Effect.CreateEffect(e:GetHandler())
	ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	ge1:SetCode(EVENT_SSET)
	ge1:SetOperation(c65010015.setflagop)
	Duel.RegisterEffect(ge1,0)
end
function c65010015.setflagop(e,tp,eg,ep,ev,re,r,rp)
	local ec=eg:GetFirst()
	while ec do
		ec:RegisterFlagEffect(65010015,RESET_EVENT+RESETS_STANDARD,0,1)
		ec=eg:GetNext()
	end
end
...
function c65010015.con(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetTurnPlayer()~=e:GetHandlerPlayer()
		 and rp==tp and re:IsActiveType(TYPE_SPELL+TYPE_TRAP) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and rc:GetFlagEffect(65010015)==0
end
