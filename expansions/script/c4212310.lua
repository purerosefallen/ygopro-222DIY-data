...
function c4212310.initial_effect(c)
    ...
    c:RegisterEffect(e0)
    --synchro level koishi only
    local e1=Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_SINGLE)
    e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
    e1:SetCode(EFFECT_ALLOW_SYNCHRO_KOISHI)
    e1:SetValue(function(e,c) return e:GetHandler():GetRank() end)
    c:RegisterEffect(e1)
    --copy spell
...
function c4212310.spcon(e,c)    
    ...
    return ft>0 and Duel.IsExistingMatchingCard(c4212310.cfilter,tp,LOCATION_SZONE,0,3,nil) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false,POS_FACEUP,tp) and Duel.GetLocationCountFromEx(tp)>0
end
function c4212310.spop(e,tp,eg,ep,ev,re,r,rp,c)
    local c = e:GetHandler()
    local sg = Duel.SelectMatchingCard(tp,c4212310.cfilter,tp,LOCATION_SZONE,0,3,3,nil)
    c:SetMaterial(sg)
    Duel.Overlay(c,sg)
