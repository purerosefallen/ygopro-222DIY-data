...
function c47530032.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg:GetCount()>0 and Duel.SendtoHand(sg,nil,REASON_EFFECT)~=0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then 
		local g1=Duel.GetMatchingGroup(c47530032.spfilter1,tp,LOCATION_HAND,0,nil,e,tp)
		if #g1>0 and Duel.SelectYesNo(tp,aux.Stringid(47530032,3)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
