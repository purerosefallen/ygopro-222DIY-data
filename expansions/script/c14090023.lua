...
function cm.filter1(c)
	return c:IsFaceup() and c:IsLevelAbove(5) and c:IsRace(RACE_PLANT)
end
...
function cm.cfilter(c,code)
	return c:IsCode(code) and c:IsAbleToHand() and c:IsAbleToGrave()
end
...
function cm.dscon(e)
	return e:GetHandler():GetPreviousLevelOnField()>=8
end