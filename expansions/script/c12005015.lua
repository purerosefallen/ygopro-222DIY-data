...
function c12005015.sprfilter1(c,tp)
    return c:IsAbleToRemoveAsCost() and c:IsType(TYPE_COUNTER) and c:IsSetCard(0xfbb) and Duel.IsExistingMatchingCard(c12005015.sprfilter2,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_ONFIELD,0,1,nil,tp,c:GetCode())
end
function c12005015.sprfilter2(c,tp,code)
    return c:IsAbleToRemoveAsCost() and c:IsType(TYPE_COUNTER) and c:IsSetCard(0xfbb) and not c:IsCode(code) and Duel.IsExistingMatchingCard(c12005015.sprfilter3,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_ONFIELD,0,1,nil,c:GetCode(),code)
end
...
function c12005015.sprop(e,tp,eg,ep,ev,re,r,rp,c)
    ...
    local tg1=Duel.SelectMatchingCard(tp,c12005015.sprfilter1,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_ONFIELD,0,1,1,nil,tp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
    local tg2=Duel.SelectMatchingCard(tp,c12005015.sprfilter2,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_ONFIELD,0,1,1,nil,tp,tg1:GetFirst():GetCode())
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
    local tg3=Duel.SelectMatchingCard(tp,c12005015.sprfilter3,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_ONFIELD,0,1,1,nil,tg1:GetFirst():GetCode(),tg2:GetFirst():GetCode())
    tg1:Merge(tg2)
    tg1:Merge(tg3)
