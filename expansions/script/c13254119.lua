...
function c13254119.filter5a(c,e)
...
function c13254119.target1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg1=Duel.GetFusionMaterial(tp):Filter(c13254119.filter0a,nil)
		local mg2=Duel.GetMatchingGroup(c13254119.filter0a,tp,0,LOCATION_MZONE,nil)
		local mg3=Duel.GetMatchingGroup(c13254119.filter3a,tp,LOCATION_GRAVE,LOCATION_GRAVE,nil)
		mg1:Merge(mg2)
		mg1:Merge(mg3)
		local res=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c13254119.filter2a),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp,mg1,nil,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c13254119.filter2a),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c13254119.activate1(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg3=Duel.GetMatchingGroup(c13254119.filter3a,tp,LOCATION_GRAVE,LOCATION_GRAVE,nil,e)
	mg1:Merge(mg2)
	mg1:Merge(mg3)
	local sg1=Duel.GetMatchingGroup(aux.NecroValleyFilter(c13254119.filter2a),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,nil,e,tp,mg1,nil,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(aux.NecroValleyFilter(c13254119.filter2a),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,nil,e,tp,mg2,mf,chkf)
	end
