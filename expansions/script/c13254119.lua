...
function c13254119.target1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg2=Duel.GetMatchingGroup(c13254119.filter6a,tp,0,LOCATION_MZONE,nil)
		local mg3=Duel.GetMatchingGroup(c13254119.filter3a,tp,LOCATION_GRAVE,LOCATION_GRAVE,nil)
		mg1:Merge(mg2)
		mg1:Merge(mg3)
		local res=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c13254119.filter2a),tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,nil,e,tp,mg1,nil,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c13254119.filter2a),tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
