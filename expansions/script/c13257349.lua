...
function c13257349.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ep~=tp then
		...
		if not eg:IsImmuneToEffect(e) then
			...
			eg:RegisterFlagEffect(23257349,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fid)
			local e1=Effect.CreateEffect(c)
			...
			if eg:IsType(TYPE_TRAPMONSTER) then
				local e3=Effect.CreateEffect(c)
				e3:SetType(EFFECT_TYPE_SINGLE)
				e3:SetCode(EFFECT_DISABLE_TRAPMONSTER)
				e3:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
				eg:RegisterEffect(e3)
			end
			local e4=Effect.CreateEffect(c)
...
function c13257349.operation1(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	while tc do
		c13257349.disop(e,tp,tc,ep,ev,re,r,rp)
		tc=g:GetNext()
	end
end
