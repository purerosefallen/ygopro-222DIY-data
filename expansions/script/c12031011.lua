...
function c12031011.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.GetFlagEffect(tp,12031011)==0 and rp==1-tp then return end
	if not re:GetHandler():IsRelateToEffect(e)  then
...
function c12031011.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SelectMatchingCard(tp,c12031011.thfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,cc,cc,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	Duel.DiscardHand(tp,nil,cc-1,cc-1,nil)
	else
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	Duel.SelectMatchingCard(tp,c12031011.thfilter1,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,cc,cc,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	Duel.DiscardHand(tp,nil,cc-1,cc-1,nil)
	end
end