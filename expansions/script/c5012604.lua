...
function c5012604.spop(e,tp,eg,ep,ev,re,r,rp)
	local at=Duel.GetAttackTarget()
	if at and at:IsRelateToBattle() and Duel.SendtoHand(at,nil,REASON_EFFECT)>0 and Duel.SpecialSummon(e:GetHandler(),0,tp,tp,false,false,POS_FACEUP)>0 then
...
function c5012604.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS):Filter(c5012604.filter,nil,tp) 
	if chk==0 then return g:GetCount()>0 and Duel.GetMZoneCount(tp,g)>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
