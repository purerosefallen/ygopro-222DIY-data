...
function c81010014.initial_effect(c)
    ...
    e3:SetTarget(c81010014.target)
    e3:SetValue(-1500)
    c:RegisterEffect(e3)
    --to grave
    local e4=Effect.CreateEffect(c)
    e4:SetDescription(aux.Stringid(81010014,1))
    e4:SetCategory(CATEGORY_DAMAGE+CATEGORY_TOGRAVE)
    e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
    e4:SetCode(EVENT_BATTLE_START)
    e4:SetTarget(c81010014.tgtg)
    e4:SetOperation(c81010014.tgop)
    c:RegisterEffect(e4)
    --attack all
...
function c81010014.target(e,c)
    return c:IsFaceup() and not c:IsType(TYPE_RITUAL)
end
function c81010014.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c81010014.tgop(e,tp,eg,ep,ev,re,r,rp)
