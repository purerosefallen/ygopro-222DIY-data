...
function c47591855.spfilter3(c)
    return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_SPELLCASTER) and c:IsType(TYPE_TUNER) and c:IsLevel(5)
end
function c47591855.spfilter1(c,tp,g)
    return g:IsExists(c47591855.spfilter2,1,c,tp,c)
end
function c47591855.spfilter2(c,tp,mc)
    return Duel.GetLocationCountFromEx(tp,tp,Group.FromCards(c,mc))>0
end
function c47591855.spcon(e,c)
    if c==nil then return true end
    local tp=c:GetControler()
    local g=Duel.GetReleaseGroup(tp):Filter(c47591855.spfilter3,nil,c)
    return g:IsExists(c47591855.spfilter1,1,nil,tp,g) and c:IsFacedown()
end
function c47591855.spop(e,tp,eg,ep,ev,re,r,rp,c)
    local g=Duel.GetReleaseGroup(tp):Filter(c47591855.spfilter3,nil,c)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
    local g1=g:FilterSelect(tp,c47591855.spfilter1,1,1,nil,tp,g)
    Duel.Release(g1,REASON_COST+REASON_SYNCHRO+REASON_MATERIAL)
end
