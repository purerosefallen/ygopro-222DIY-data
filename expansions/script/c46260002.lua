...
function c46260002.negcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroupEx(tp,aux.NOT(aux.FilterBoolFunction(Card.IsStatus,STATUS_BATTLE_DESTROYED)),1,nil) end
	local g=Duel.SelectReleaseGroupEx(tp,aux.NOT(aux.FilterBoolFunction(Card.IsStatus,STATUS_BATTLE_DESTROYED)),1,1,nil)
