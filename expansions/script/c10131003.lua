...
function c10131003.pcfilter(c,e,tp)
	return c:IsSetCard(0x5338) and c:IsType(TYPE_PENDULUM) and c:GetPreviousControler()==tp and c:IsReason(REASON_EFFECT)  and c:IsPreviousSetCard(0x5338) and c:IsPreviousPosition(POS_FACEUP) and bit.band(c:GetPreviousTypeOnField(),TYPE_PENDULUM)~=0 and c:IsPreviousLocation(LOCATION_ONFIELD) and ((c:IsCanBeSpecialSummoned(e,0,tp,false,false) and ((c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp)>0) or (c:IsLocation(LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0))) or (not c:IsForbidden() and (Duel.CheckLocation(tp,LOCATION_PZONE,0) or Duel.CheckLocation(tp,LOCATION_PZONE,1))))
end
...
function c10131003.pcop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		if ft>0 and tc:IsCanBeSpecialSummoned(e,0,tp,false,false)
			and (not pc or not Duel.SelectYesNo(tp,aux.Stringid(10131003,2))) then
...
function c10131003.spfilter(c,e,sp)
	return c:IsSetCard(0x5338) and c:GetLevel()==4 and c:IsCanBeSpecialSummoned(e,0,sp,false,false) and (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsType(TYPE_PENDULUM) and ((c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp)>0) or (c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0))
end
function c10131003.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c10131003.spfilter,tp,LOCATION_HAND+LOCATION_EXTRA,0,1,nil,e,tp) end
