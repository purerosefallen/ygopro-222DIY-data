...
function cm.adcon(e)
	...
	return ((a and a:IsControler(tp) and cm.Skay(a)) or (d and d:IsControler(tp) and cm.Skay(d))) and (Duel.GetCurrentPhase()==PHASE_DAMAGE or Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL) and (c:GetSummonType()==SUMMON_TYPE_SPECIAL+1 or c:GetFlagEffect(m)~=0)
end
