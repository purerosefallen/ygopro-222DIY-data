...
function c22222102.activate(e,tp,eg,ep,ev,re,r,rp)
	if not (Duel.GetMZoneCount(tp)>0 and Duel.IsExistingMatchingCard(c22222102.filter,tp,LOCATION_DECK,0,1,nil,e,tp)) then return end
	local sc=Duel.SelectMatchingCard(tp,c22222102.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp):GetFirst()
