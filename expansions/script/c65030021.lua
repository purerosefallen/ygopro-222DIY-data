--护佑的二重阴影
function c65030021.initial_effect(c)
	aux.EnableDualAttribute(c)
	--SPSUMMON
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(65030021,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND)
	e1:SetCondition(c65030021.spcon)
	e1:SetTarget(c65030021.sptg)
	e1:SetOperation(c65030021.spop)
	c:RegisterEffect(e1)
	--effect!
	local e5=Effect.CreateEffect(c)
	e5:SetCategory(CATEGORY_TOGRAVE+CATEGORY_SPECIAL_SUMMON+CATEGORY_TODECK)
	e5:SetType(EFFECT_TYPE_IGNITION)
	e5:SetRange(LOCATION_MZONE)
	e5:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e5:SetCountLimit(1)
	e5:SetCondition(aux.IsDualState)
	e5:SetTarget(c65030021.tg)
	e5:SetOperation(c65030021.op)
	c:RegisterEffect(e5)
end
function c65030021.tgfilter(c,tp)
	local cd=c:GetCode()
	return c:IsFaceup() and c:IsType(TYPE_NORMAL) and Duel.IsExistingMatchingCard(c65030021.tgfil2,tp,LOCATION_DECK,0,2,nil,cd) 
end
function c65030021.tgfil2(c,code)
	return not c:IsCode(code) and c:IsAbleToGrave() and c:IsType(TYPE_MONSTER) 
end
function c65030021.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return c65030021.tgfilter(chkc,tp) and chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) end
	if chk==0 then return Duel.IsExistingTarget(c65030021.tgfilter,tp,LOCATION_MZONE,0,1,nil) end
	local g=Duel.SelectTarget(tp,c65030021.tgfilter,tp,LOCATION_MZONE,0,1,1,nil,tp)
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_DECK)
end
function c65030021.thfilter(c,e,tp)
	return c:IsSetCard(0xcda1) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c65030021.op(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	local code=tc:GetCode()
	if tc:IsRelateToEffect(e) and Duel.IsExistingMatchingCard(c65030021.tgfil2,tp,LOCATION_DECK,0,2,nil,code) then
		local g=Duel.SelectMatchingCard(tp,c65030021.tgfil2,tp,LOCATION_DECK,0,2,2,nil,code)
		if Duel.SendtoGrave(g,REASON_EFFECT)~=0 then
			Duel.BreakEffect()
			local mg=Duel.GetOperatedGroup()
			if mg:FilterCount(Card.IsType,nil,TYPE_NORMAL)==mg:GetCount() and Duel.IsExistingMatchingCard(c65030021.thfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) and Duel.SelectYesNo(tp,aux.Stringid(65030021,0)) then
			local sg=Duel.SelectMatchingCard(tp,c65030021.thfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
			Duel.SpecialSummon(sg,0,tp,tp,false,false,POS_FACEUP)
			elseif mg:FilterCount(Card.IsType,nil,TYPE_NORMAL)~=mg:GetCount() then
				Duel.SendtoDeck(mg,nil,2,REASON_EFFECT)
			end
		end
	end
end
function c65030021.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetMatchingGroupCount(Card.IsType,tp,LOCATION_MZONE,0,nil,TYPE_EFFECT)==0 
end
function c65030021.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,tp,LOCATION_HAND)
end
function c65030021.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
	end
end