...
function c60150543.filter2(c)
    return c:IsFaceup() and c:IsType(TYPE_EFFECT) and c:GetAttack()>0 and not c:IsDisabled()
end
...
function c60150543.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:GetAttack()>0 
		and not c:IsDisabled() and not tc:IsImmuneToEffect(e) then
        Duel.NegateRelatedChain(tc,RESET_TURN_SET)
        local e3=Effect.CreateEffect(c)
		e3:SetDescription(aux.Stringid(60150543,2))
