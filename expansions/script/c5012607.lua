...
function c5012607.refilter(c)
    return (c:IsSetCard(0x250) or c:IsSetCard(0x35a)) and c:IsAbleToRemoveAsCost()
end
...
function c5012607.aclimit(e,re,tp)
    return not (re:GetHandler():IsSetCard(0x250) or re:GetHandler():IsSetCard(0x35a))
end
...
function c5012607.cfilter(c)
    return c:IsFaceup() and (c:IsSetCard(0x250) or c:IsSetCard(0x35a))
end
...
function c5012607.spfilter2(c,e,tp)
    return c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,true,POS_FACEUP,1-tp) and c:IsCode(5012604) and ((c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(1-tp)>0) or (not c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0))
end
function c5012607.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    Duel.SelectYesNo(tp,aux.Stringid(5012607,2)) then
       ...
       Duel.SpecialSummon(tg,SUMMON_TYPE_SYNCHRO,tp,1-tp,false,true,POS_FACEUP)
