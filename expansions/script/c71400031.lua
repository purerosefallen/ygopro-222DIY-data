...
function c71400031.filter(c)
	return c:IsCardSet(0x714) and c:GetReason() & REASON_DESTROY~=0 and not c:IsCode(71400031) and c:IsAbleToHand()
end
function c71400031.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.GetMatchingCard(tp,c71400031.filter,tp,LOCATION_GRAVE,0,1,1,nil)
...
function c71400031.op(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SetLP(tp,Duel.GetLP(tp)-ct*2000)
end