...
function c47530018.filter(c)
    return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:GetOverlayCount()>0
end
function c47530018.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_MZONE) and c47530018.filter(chkc) end
    if chk==0 then return Duel.IsExistingTarget(c47530018.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
    local g=Duel.SelectTarget(tp,c47530018.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
