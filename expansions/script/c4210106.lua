...
function c4210106.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,nil,4,2,function(c)return c:IsFaceup()and c:IsSetCard(0x2ac)and c:IsType(TYPE_MONSTER) and not c:IsCode(4210106) end,aux.Stringid(4210106,0),2,function(e,tp,chk)if chk==0 then return Duel.GetFlagEffect(tp,4210106)==0 end	Duel.RegisterFlagEffect(tp,4210106,RESET_PHASE+PHASE_END,0,1)end)
	c:EnableReviveLimit()	
	--atkup
...
function c4210106.mattg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(function(c)return c:IsSetCard(0x2ac) and c:IsType(TYPE_MONSTER) end,tp,LOCATION_HAND,0,1,nil) end
