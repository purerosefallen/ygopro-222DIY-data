...
function cm.sumcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsSummonType(SUMMON_TYPE_LINK) and c:GetMaterial():IsExists(Card.IsCode,1,nil,14010051)
end
...
function cm.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(tc,REASON_EFFECT)~=0 and g:GetCount()>0 and tc:IsFaceup() and tc:IsRace(RACE_ZOMBIE) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0  and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
		Duel.BreakEffect()
