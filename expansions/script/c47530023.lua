...
function c47530023.initial_effect(c)
	...
	e3:SetCondition(c47530023.discon2)
	e3:SetTarget(c47530023.distg2)
	e3:SetOperation(c47530023.disop2)
	c:RegisterEffect(e3)
end
...
function c47530023.dfilter(c,nseq)
	return c:GetSequence()==nseq
end
function c47530023.con(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c47530023.dfilter,1,nil,e:GetLabel())
end
function c47530023.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetCard(eg)
	local g=eg:Filter(c47530023.dfilter,nil,e:GetLabel())
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,g,g:GetCount(),0,0)
end
function c47530023.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=eg:Filter(c47530023.dfilter,nil,e:GetLabel()):Filter(Card.IsRelateToEffect,nil,e)
	local tc=g:GetFirst()
	if tc then
		local e1=Effect.CreateEffect(c)
...
function c47530023.discon2(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev)
end
function c47530023.distg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
...
function c47530023.disop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
