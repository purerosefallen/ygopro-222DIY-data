...
function c47578926.initial_effect(c)
    ...
    e2:SetCondition(c47578926.thcon)
    e2:SetTarget(c47578926.thtg)
    e2:SetOperation(c47578926.thop)
    c:RegisterEffect(e2)
    --serch
    ...
    e3:SetTarget(c47578926.thtg2)
    e3:SetOperation(c47578926.thop2)
    c:RegisterEffect(e3)
    local e4=e3:Clone()
    ...
    e6:SetCountLimit(1)
    e6:SetTarget(c47578926.tg)
    e6:SetOperation(c47578926.op)
    c:RegisterEffect(e6)
end
...
function c47578926.filter(c)
    return c:IsRace(RACE_FAIRY) and c:IsAbleToGrave() 
end
function c47578926.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    local g=Duel.SelectTarget(tp,c47578926.filter,tp,LOCATION_DECK,0,1,1,nil)
...
function c47578926.atktg(e,c)
    return c:IsRace(RACE_FAIRY)
end
function c47578926.tg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return true end
    if not Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) then
        Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,500)
    end
end
function c47578926.op(e,tp,eg,ep,ev,re,r,rp)
