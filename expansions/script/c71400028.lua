...
function c71400028.tg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and c71400028.filter1(chkc,e,tp) end
	if chk==0 then return zone~=0 and Duel.IsExistingTarget(c71400028.filter1,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil,e,tp,zone) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c71400028.filter1,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil,e,tp,zone)
...
function c71400028.tg3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCountFromEx(tp)>0
		and Duel.IsExistingMatchingCard(c71400028.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
