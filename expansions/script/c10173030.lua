...
function c10173030.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_NEGATE+CATEGORY_REMOVE+CATEGORY_DAMAGE+CATEGORY_DESTROY)
...
function c10173030.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,g:GetCount(),0,0)   Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,g:GetCount()*200) 
...
function c10173030.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateActivation(ev) and g:GetCount()>0 then
	   ...
	   if ct>0 then
		  Duel.BreakEffect()
		  if Duel.Damage(1-tp,ct*200,REASON_EFFECT)~=0 and c:IsRelateToEffect(e) then
