...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        local s=Duel.GetFieldGroupCount(tp,LOCATION_PZONE,0) 
    if s>0 and not Duel.IsPlayerCanDraw(tp,s) then return end
        local chkf=tp
    ...
    Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,s)
