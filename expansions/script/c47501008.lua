...
function c47501008.initial_effect(c)
    ...
    e0:SetCondition(c47501008.spcon)
    e0:SetOperation(c47501008.spop)
    c:RegisterEffect(e0) 
    --splimit
...
function c47501008.ffilter(c)
    return c:IsCode(47500000) and c:IsFaceup()
end
function c47501008.spfilter(c,fc)
    return c47501008.ffilter(c) and c:IsCanBeFusionMaterial(fc)
end
function c47501008.spfilter1(c,tp,g)
    return g:IsExists(c47501008.spfilter2,1,c,tp,c)
end
function c47501008.spfilter2(c,tp,mc)
    return Duel.GetLocationCountFromEx(tp,tp,Group.FromCards(c,mc))>0
end
function c47501008.spcon(e,c)
    if c==nil then return true end
    local tp=c:GetControler()
    local g=Duel.GetMatchingGroup(c47501008.ffilter,tp,LOCATION_EXTRA,0,nil)
    return g:IsExists(c47501008.spfilter1,1,nil,tp,g) and c:IsFacedown()
end
function c47501008.spop(e,tp,eg,ep,ev,re,r,rp,c)
    local g=Duel.GetMatchingGroup(c47501008.ffilter,tp,LOCATION_EXTRA,0,nil)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
    local g1=g:FilterSelect(tp,c47501008.spfilter1,1,1,nil,tp,g)
    local mc=g1:GetFirst()
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
    local g2=g:FilterSelect(tp,c47501008.spfilter2,1,1,mc,tp,mc)
    g1:Merge(g2)
