...
function cm.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	if rc:IsRelateToEffect(re) and Duel.Destroy(eg+e:GetHandler(),REASON_EFFECT)>1 and not rc:IsLocation(LOCATION_HAND+LOCATION_DECK)
...
function cm.filter1(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xfb2) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.filter1,tp,LOCATION_SZONE,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
