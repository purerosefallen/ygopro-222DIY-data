...
function c22250001.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if Duel.IsPlayerAffectedByEffect(tp,EFFECT_MUST_BE_FMATERIAL) then return false end
	local tc=eg:GetFirst()
...
function c22250001.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerAffectedByEffect(tp,EFFECT_MUST_BE_FMATERIAL) then return end
	local c=e:GetHandler()
	local tc=eg:GetFirst()
	local mg=Group.FromCards(c,tc)
