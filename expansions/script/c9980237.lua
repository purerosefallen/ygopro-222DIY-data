...
function c9980237.initial_effect(c)
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c9980237.condition)
	e1:SetTarget(c9980237.target)
	e1:SetOperation(c9980237.activate)
	c:RegisterEffect(e1)
	--special summon
...
function c9980237.condition(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	return ph==PHASE_MAIN1 or (ph>=PHASE_BATTLE_START and ph<=PHASE_BATTLE) or ph==PHASE_MAIN2
end
function c9980237.thfilter(c)
	return c:IsSetCard(0x1bc4) and c:IsType(TYPE_MONSTER) and c:IsFaceup() and c:IsAbleToHand()
end
function c9980237.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c9980237.activate(e,tp,eg,ep,ev,re,r,rp)
...
function c9980237.filter(c,e,tp)
	return c:IsSetCard(0x1bc4) and c:IsType(TYPE_MONSTER) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_ATTACK)
end
