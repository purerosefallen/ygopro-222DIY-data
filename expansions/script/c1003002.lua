...
function c1003002.initial_effect(c)
   --xyz summon
	...
	e2:SetTarget(c1003002.reptg)
	c:RegisterEffect(e2)
	--damage
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(1003002,1))
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e2:SetCategory(CATEGORY_DAMAGE)
	e2:SetCode(EVENT_DETACH_MATERIAL)
	e2:SetCondition(c1003002.damcon)
	e2:SetTarget(c1003002.damtg)
	e2:SetOperation(c1003002.damop)
	c:RegisterEffect(e2)
	--disable
...
function c1003002.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.SelectYesNo(tp,aux.Stringid(1003002,0)) then
		e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_EFFECT)
		return true
	else return false end
end
function c1003002.damcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetOverlayCount()==0
end
function c1003002.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsRelateToEffect(e) and e:GetHandler():IsFaceup() end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetTargetParam(1000)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,0,0,1-tp,1000)
end
function c1003002.damop(e,tp,eg,ep,ev,re,r,rp)
