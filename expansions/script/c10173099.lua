...
function c10173099.tdfilter(c,e,tp)
	return c:IsFaceup() and c:IsAbleToExtra() and c:IsType(TYPE_LINK) and Duel.IsExistingMatchingCard(c10173099.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c) and Duel.GetLocationCountFromEx(tp,tp,c)>0
end
function c10173099.spfilter(c,e,tp,rc)
	if c:GetLink()~=rc:GetLink() or not c:IsCanBeSpecialSummoned(e,0,tp,false,false) then return false end
...
function c10173099.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c10173099.tdfilter,tp,LOCATION_MZONE,0,1,nil,e,tp) end
...
function c10173099.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc,nil,2,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp)>0 and Duel.IsExistingMatchingCard(c10173099.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,tc) then
	   Duel.BreakEffect()
