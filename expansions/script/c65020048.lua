...
function c65020048.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetMatchingGroupCount(c65020048.tdfil,tp,LOCATION_REMOVED,0,nil)>=5 end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,5,tp,LOCATION_REMOVED)
end
function c65020048.op(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetMatchingGroupCount(c65020048.tdfil,tp,LOCATION_REMOVED,0,nil)<5 then return end
	local g=Duel.SelectMatchingCard(tp,c65020048.tdfil,tp,LOCATION_REMOVED,0,5,5,nil)
	if Duel.SendtoDeck(g,nil,2,REASON_EFFECT)==5 and Duel.IsPlayerCanDraw(tp) and Duel.SelectYesNo(tp,aux.Stringid(65020048,0)) then
		Duel.BreakEffect()
...
function c65020048.abletoremove(c)
	return c:IsAbleToRemove() and not c:IsType(TYPE_TOKEN)
end
function c65020048.reatg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c65020048.abletoremove,tp,LOCATION_ONFIELD+LOCATION_GRAVE,LOCATION_ONFIELD+LOCATION_GRAVE,1,nil) end
...
function c65020048.reaop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(tp,c65020048.abletoremove,tp,LOCATION_ONFIELD+LOCATION_GRAVE,LOCATION_ONFIELD+LOCATION_GRAVE,1,1,nil)
