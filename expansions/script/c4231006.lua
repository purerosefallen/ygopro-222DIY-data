...
function cm.initial_effect(c)--red
    ...
    .Return()).c("RegisterEffect",iFunc(c)
        ...
        .e("SetValue",function(e,te)
            ...
            local sg = Group.FromCards(te:GetOwner())
            return not ((te:GetHandler():IsType(TYPE_EQUIP) or te:GetHandler():IsType(TYPE_RITUAL)) and te:GetHandler():IsType(TYPE_SPELL)) and not (g==nil and {(te:GetOwner()~=e:GetHandler():GetEquipTarget() and {false} or {true})[1]} or {g:IsContains(e:GetHandler():GetEquipTarget())})[1] end)
    .Return()).c("RegisterEffect",iFunc(c)
...
Duel.RegisterFlagEffect(tp,m,RESET_PHASE+PHASE_END,0,1) end)
        .e("SetOperation",function(e,tp,eg,ep,ev,re,r,rp)
            local g = Duel.SelectMatchingCard(tp,cm.cfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,4231001,e,tp)
