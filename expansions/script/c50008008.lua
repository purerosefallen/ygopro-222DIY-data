...
function cm.filter(c)
    return c:IsRace(RACE_SPELLCASTER) and c:GetAttack()==1850 and c:IsSummonable(true,nil)
end
...
function cm.tgfilter(c)
    return c:IsRace(RACE_SPELLCASTER) and c:GetAttack()==1850 and c:IsAbleToGrave()
end
...
function cm.tgop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if g:GetCount()>0 then
        if Duel.SendtoGrave(g,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
            Duel.BreakEffect()
            Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
            local tg=Duel.SelectTarget(tp,Card.IsAbleToDeck,tp,0,LOCATION_GRAVE,1,1,nil)
...
function cm.tgop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.SelectMatchingCard(tp,cm.tgfilter,tp,LOCATION_DECK,0,1,1,nil)
    if g:GetCount()>0 then
        if Duel.SendtoGrave(g,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,LOCATION_GRAVE,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
            Duel.BreakEffect()
            Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
