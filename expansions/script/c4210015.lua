...
function c4210015.spcon(e,c)	
	...
	local rg=Duel.GetReleaseGroup(tp):Filter(c4210015.spfilter,nil,ft,tp)
	local rgc=rg:GetCount()
	local c=e:GetHandler()
	return (rgc>0 and Duel.CheckReleaseGroup(tp,c4210015.spfilter,1,nil,rgc,tp) and c:IsLocation(LOCATION_HAND))
		or (rgc>1 and Duel.CheckReleaseGroup(tp,c4210015.spfilter,2,nil,rgc,tp) and c:IsLocation(LOCATION_DECK))
end
function c4210015.spop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	elseif ft==0 then
		...
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
		g=rg:FilterSelect(tp,c4210015.mzfilter,rec,rec,nil,tp)
	end
